...
namespace Ultima
{
	...
	public sealed class Client
	{
		...
		private static string GetExePath( string subName )
		{
			try
			{
                RegistryKey key = Registry.LocalMachine.OpenSubKey(String.Format(@"SOFTWARE\{0}Origin Worlds Online\{1}\1.0", IntPtr.Size == 8 ? "Wow6432Node\\" : "", subName));
				
                if (key == null)
                {
                    key = Registry.LocalMachine.OpenSubKey(String.Format(@"SOFTWARE\{0}EA Games\Ultima Online: Mondain's Legacy\1.00.0000", IntPtr.Size == 8 ? "Wow6432Node\\" : ""));
